...
import { confirmCardPaymentIfNeeded } from "@/features/bookings/api";
import {
  checkoutServiceBooking,
  completeServiceBookingCheckout,
  getServiceBooking,
...
export default function ServiceBookingCheckoutPage({
  params,
}: {
  ...
  const { data: booking, isLoading, refetch } = useQuery({
    queryKey: marketplaceKeys.booking(user?.id, bookingId),
  ...
  const syncBookingCache = async (confirmed: ServiceBooking) => {
    if (!user?.id) return;
    queryClient.setQueryData(
      marketplaceKeys.booking(user.id, bookingId),
      confirmed,
    );
  ...
  const checkoutMut = useMutation({
    mutationFn: () =>
      checkoutServiceBooking(bookingId, {
        paymentMethodId: selectedMethod?.stripePaymentMethodId,
      }),
    onSuccess: async (result) => {
      if (result.status === "requires_action") {
        try {
          const paymentIntentId = await confirmCardPaymentIfNeeded(
            result.clientSecret,
          );
          const completed = await completeServiceBookingCheckout(
            bookingId,
            paymentIntentId,
          );
          await syncBookingCache(completed.booking);
          toast.success(tVenueCheckout("paymentConfirmed"));
          router.push(`/userDashboard/service-bookings/${bookingId}`);
        } catch (e) {
      ...
      toast.success(tUserDashboard("serviceBookingConfirmed"));
      router.push(`/userDashboard/service-bookings/${bookingId}`);
    },
    onError: (e) => {
  ...
  useEffect(() => {
    if (booking?.status === "CONFIRMED") {
      router.replace(`/userDashboard/service-bookings/${bookingId}`);
    }
            ...
            <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
              <div>
                ...
                <h1 className="mt-1 text-xl font-bold text-white sm:text-3xl">
                  {tUserDashboard("completeServiceBooking")}
                </h1>
                <p className="mt-2 max-w-md text-sm text-muted-foreground">
                  {tUserDashboard("completeServiceBookingDesc")}
                </p>
              </div>
              {booking.expiresAt ? (
                <CountdownTimer
                ...
                <div className="min-w-0 flex-1 space-y-3">
                  <div>
                    <p className="font-semibold text-white">
                      {booking.service?.title ?? tUserDashboard("serviceFallback")}
                    </p>
                    {booking.service?.vendor?.vendorName ? (
