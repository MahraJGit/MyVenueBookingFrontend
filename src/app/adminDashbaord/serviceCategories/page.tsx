...
import type { ServiceCategory } from "@/features/marketplace/types";
...
export default function ServiceCategoriesPage() {
  const t = useTranslations("adminServiceCategories");
  ...
  const [viewTarget, setViewTarget] = useState<ServiceCategory | null>(null);
  ...
  const {
    ...
    queryKey: ["service-categories", debouncedSearch],
    queryFn: () =>
      listServiceCategories({
  ...
  const updateMutation = useMutation({
    mutationFn: () => {
      ...
      return updateServiceCategory(editTarget.id, {
        name: form.name.trim(),
    ...
    <DashboardPageShell>
      <DashboardPanel>
        ...
        <div className="space-y-2">
          <Label htmlFor="service-category-search" className="text-muted-foreground">
            {tCommon("search")}
          </Label>
              ...
              {isLoading ? (
                ...
                <>
                  ...
                  {!isLoading && categories.length === 0 ? (
                    <TableEmptyRow colSpan={6}>{t("noCategories")}</TableEmptyRow>
                  ) : null}
                </>
              )}
            </TableBody>
        ...
        >
          <DialogContent className={cn(dashboardDialogContentClass)}>
            ...
            <div className="space-y-4">
              <div className="space-y-2">
                <Label htmlFor="create-service-cat-name">{tCommon("name")}</Label>
                <Input
                  id="create-service-cat-name"
                  value={form.name}
                  onChange={(e) =>
                  ...
                  maxLength={80}
                />
                <p className="text-xs text-zinc-500">{t("nameHint")}</p>
              </div>
              <div className="space-y-2">
                <Label htmlFor="create-service-cat-desc">{t("descriptionOptional")}</Label>
                <Textarea
                  id="create-service-cat-desc"
        ...
        >
          <DialogContent className={cn(dashboardDialogContentClass)}>
            ...
            <div className="space-y-4">
              <div className="space-y-2">
                <Label htmlFor="edit-service-cat-name">{tCommon("name")}</Label>
                <Input
                  id="edit-service-cat-name"
                  value={form.name}
                  onChange={(e) =>
                    setForm((f) => ({ ...f, name: e.target.value }))
                  }
              ...
              <div className="space-y-2">
                <Label htmlFor="edit-service-cat-desc">{tCommon("description")}</Label>
                <Textarea
                  id="edit-service-cat-desc"
                  value={form.description}
                  onChange={(e) =>
                    setForm((f) => ({ ...f, description: e.target.value }))
                  }
          ...
          <DialogContent className={cn(dashboardDialogContentClass)}>
            ...
            {viewTarget ? (
              <div className="grid gap-3 text-sm">
                ...
                <Detail label={t("slug")} value={viewTarget.slug} />
                <Detail
          ...
          <DialogContent className={cn(dashboardDialogContentClass)}>
            ...
            {deleteTarget ? (
              <div className="rounded-lg border border-zinc-700 bg-zinc-900/40 p-3 text-sm">
                ...
                <p className="mt-2 text-zinc-400">{t("deleteConfirm")}</p>
              </div>
            ) : null}
