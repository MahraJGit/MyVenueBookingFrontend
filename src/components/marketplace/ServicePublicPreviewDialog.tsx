...
type ServicePublicPreviewDialogProps = {
  service: ManagedMarketplaceService | null;
  onClose: () => void;
  editHref?: string;
};

function buildPreviewUrl(id: string) {
  return `/marketplace/${encodeURIComponent(id)}?embed=1&preview=1`;
}

function buildPublicUrl(id: string, slug: string, preview: boolean) {
  if (preview) {
    return `/marketplace/${encodeURIComponent(id)}?preview=1`;
  }
  return `/marketplace/${encodeURIComponent(slug)}`;
}

export function ServicePublicPreviewDialog({
  service,
  onClose,
  editHref,
}: ServicePublicPreviewDialogProps) {
  const t = useTranslations("vendorMarketplace");
  const tCommon = useTranslations("common");

  const open = Boolean(service);
  const previewUrl = service ? buildPreviewUrl(service.id) : "";
  const usePreviewMode = service?.status !== "ACTIVE";
  const publicUrl = service
    ? buildPublicUrl(service.id, service.slug, usePreviewMode)
    : "";
    ...
    <Dialog open={open} onOpenChange={(next) => !next && onClose()}>
      ...
      >
        <div className="flex shrink-0 flex-col gap-3 border-b border-[#242424] px-4 py-4 sm:flex-row sm:items-center sm:justify-between sm:px-5">
          <div className="min-w-0 flex-1 space-y-1">
            ...
            <DialogDescription className="flex flex-wrap items-center gap-2 text-left text-zinc-400">
              ...
              {service?.status ? <StatusBadge status={service.status} /> : null}
            </DialogDescription>
          </div>
        ...
        {service ? (
          <iframe
            key={previewUrl}
            title={t("previewService", { title: service.title })}
            src={previewUrl}
